...
_usable = _usable call RSTF_fnc_arrayShuffle;
...
for[{_i = 0},{_i < RSTF_NEUTRALS_GROUPS},{_i = _i + 1}] do {
	...
	for[{_u = 0},{_u < _units && _u < count(_positions)},{_u = _u + 1}] do {
		...
		_pos1 = ATLtoASL(_position);
		_pos2 = _pos1 vectorAdd [0, 0, 5];
		_canCrouch = (!(lineIntersects [_pos1, _pos2]) && _canHaveStatic);
