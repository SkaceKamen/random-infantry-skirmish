...
_template = '
	[%2, {
		%2 = _this;
		["%1", %2] call RSTF_configUpdateFactions;
	}] spawn RSTF_showFactions;
';

{
	_ctrl = ["RSTF_RscDialogConfig", "edit", ["controls", _x select 0, "controls"]] call RSTF_getCtrl;
	_ctrl ctrlAddEventHandler ["ButtonClick", compile(format[_template,_x select 0,_x select 1])];
	call compile format['["%1", %2] call RSTF_configUpdateFactions',_x select 0,_x select 1];
} foreach [
...
//Game config
_ctrl = ["RSTF_RscDialogConfig", "scoreLimit", ["controls", "gameConfig", "controls"]] call RSTF_getCtrl;
_ctrl ctrlSetText str(RSTF_SCORE_LIMIT);

_ctrl = ["RSTF_RscDialogConfig", "scorePerKill", ["controls", "gameConfig", "controls"]] call RSTF_getCtrl;
_ctrl ctrlSetText str(RSTF_SCORE_PER_KILL);

_ctrl = ["RSTF_RscDialogConfig", "scorePerTask", ["controls", "gameConfig", "controls"]] call RSTF_getCtrl;
_ctrl ctrlSetText str(RSTF_SCORE_PER_TASK);

_ctrl = ["RSTF_RscDialogConfig", "groupsLimit", ["controls", "gameConfig", "controls"]] call RSTF_getCtrl;
_ctrl ctrlSetText str(RSTF_LIMIT_GROUPS);

_ctrl = ["RSTF_RscDialogConfig", "unitsLimit", ["controls", "gameConfig", "controls"]] call RSTF_getCtrl;
_ctrl ctrlSetText str(RSTF_LIMIT_UNITS);

_ctrl = ["RSTF_RscDialogConfig", "neutralsLimit", ["controls", "gameConfig", "controls"]] call RSTF_getCtrl;
_ctrl ctrlSetText str(RSTF_NEUTRALS_GROUPS);

_ctrl = ["RSTF_RscDialogConfig", "spawnTime", ["controls", "gameConfig", "controls"]] call RSTF_getCtrl;
_ctrl ctrlSetText str(RSTF_LIMIT_SPAWN);


//Spawn config
_ctrl = ["RSTF_RscDialogConfig", "spawnType", ["controls", "spawnConfig", "controls"]] call RSTF_getCtrl;
lbClear _ctrl;
{
	_ctrl lbAdd _x;
...
_ctrl = ["RSTF_RscDialogConfig", "randomizeWeapons", ["controls", "spawnConfig", "controls"]] call RSTF_getCtrl;
_ctrl ctrlSetChecked RSTF_RANDOMIZE_WEAPONS;

_ctrl = ["RSTF_RscDialogConfig", "restrictWeapons", ["controls", "spawnConfig", "controls"]] call RSTF_getCtrl;
_ctrl ctrlSetChecked RSTF_RANDOMIZE_WEAPONS_RESTRICT;

_ctrl = ["RSTF_RscDialogConfig", "enableCustom", ["controls", "spawnConfig", "controls"]] call RSTF_getCtrl;
_ctrl ctrlSetChecked RSTF_CUSTOM_EQUIPMENT;

_ctrl = ["RSTF_RscDialogConfig", "changeCustom", ["controls", "spawnConfig", "controls"]] call RSTF_getCtrl;
_ctrl ctrlAddEventHandler ["ButtonClick", {
	...
	call RSTF_loadWeapons;
	call RSTF_loadClasses;
	[] spawn RSTF_showEquip;
	true;
}];

//Misc config
_ctrl = ["RSTF_RscDialogConfig", "clearDeadBodies", ["controls", "otherConfig", "controls"]] call RSTF_getCtrl;
_ctrl ctrlSetChecked RSTF_CLEAN;

_ctrl = ["RSTF_RscDialogConfig", "weather", ["controls", "otherConfig", "controls"]] call RSTF_getCtrl;
lbClear _ctrl;
{
	_ctrl lbAdd _x;
...
_ctrl = ["RSTF_RscDialogConfig", "time", ["controls", "otherConfig", "controls"]] call RSTF_getCtrl;
lbClear _ctrl;
{
	_ctrl lbAdd _x;
...
/*_ctrl = ["RSTF_RscDialogConfig", "switchIsland", ["controls", "otherConfig", "controls"]] call RSTF_getCtrl;
_ctrl ctrlAddEventHandler ["ButtonClick", {
	//'Altis' spawn RSTF_switchIsland;
	[] spawn RSTF_mapsShow;
}];*/

_ctrl = ["RSTF_RscDialogConfig", "reset", ["controls", "otherConfig", "controls"]] call RSTF_getCtrl;
_ctrl ctrlAddEventHandler ["ButtonClick", {
	[] spawn {
		if (["Do you really want to reset all configuration to default values?", "Reset", "Yes", "No"] call BIS_fnc_GUImessage) then {
			call RSTF_profileReset;
			closeDialog 0;
			sleep 0.5;
			call RSTF_showConfig;
		};
	};
}];

_ctrl = ["RSTF_RscDialogConfig", "start", ["controls", "otherConfig", "controls"]] call RSTF_getCtrl;
_ctrl ctrlAddEventHandler ["ButtonClick", {
	disableSerialization;

	_errors = call RSTF_configValidate;
	...
	*/
	...
	//	RSTF_SELECTED_WORLD spawn RSTF_switchIsland;
	//} else {
		closeDialog 1;
		[] spawn RSTF_start;
	//};
