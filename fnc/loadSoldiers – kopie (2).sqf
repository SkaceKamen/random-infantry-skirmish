		...
		if (_scope == 2 && _man == 1 && _faction in _factions) then {
			...
			if (_weaponized) then {
				if (_ignore_bans || !(configName(_c) in RSTF_SOLDIERS_BANNED)) then {
					_vehClass = getText(_c >> "vehicleClass");
					_list = [_soldiers, _vehClass, -1] call AMAP_get;
					
					if (typeName(_list) == typeName(-1)) then {
						_list = [];
						[_soldiers, _vehClass, _list] call AMAP_set;
					};
					
					_list set [count(_list), configName(_c)];
				};
			};
		};
	};
